...
def search_by_l2_distance(query, gallery, gallery_labels):
    ...
    _, best_idxs = torch.sort(torch.tensor(l2_dist_lst), descending=True)
    print(f"dist: {l2_dist_lst}")
    print(f"best idxs: {best_idxs}")
    best_pids = gallery_labels[best_idxs]
    best_pids = best_pids.clone().detach()
    print(f"best pids: {best_pids}")
    return best_pids
...
def search_on_gallery_topk(queries, query_labels, gallery, gallery_labels, metric='euclidean', topk=[1, 5]):
    ...
    if metric == 'euclidean':
        for query in queries:
            ...
            batch_pred_pids.append(pred_pids)
    elif metric == 'cosine':
        for query in queries:
    ...
    for k in topk:
        ...
        best_preds = torch.tensor(batch_pred_pids)
...
def search_on_gallery_map(queries, query_labels, gallery, gallery_labels, metric='euclidean', topk=[1, 5]):
    ...
    for k in topk:
        ...
        true_pred_queries = (batch_preds[:, :k] == query_labels[:, None])
